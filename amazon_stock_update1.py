...
def generateXmlFile(dicList):
    ...
    # for count, item in enumerate(d for d in dicList):
    #     print("count----", count)
    #     print("item-----", item)
    ...
    for count, item in enumerate(dicList, 1):
        ...
        OperationType = SubElement(message, 'OperationType')
        OperationType.text = "Update"

        Inventory_value = SubElement(message, 'Inventory')

        sku_value = SubElement(Inventory_value, 'SKU')
        sku_value.text = str(item['amazon_portal_sku'])

        quantity_value = SubElement(Inventory_value, 'Quantity')
        quantity_value.text = str(item['stock'])

        FulfillmentLatency_value = SubElement(Inventory_value, 'FulfillmentLatency')
        FulfillmentLatency_value.text = "2"

        SwitchFulfillmentTo_value = SubElement(Inventory_value, 'SwitchFulfillmentTo')
        SwitchFulfillmentTo_value.text = "MFN"
...
def processing_request(singleRequest, feed, logger, products_cursor, conn_products, writer):
    ...
    try:
        ...
        print("generatedunction__single---request--", singleRequest)
        ...
        feed_response = feed.submit_feed(tostring(xmlData), "_POST_INVENTORY_AVAILABILITY_DATA_", 'A21TJRUUN4KGV')
        ...
        print("feed_response_data--->>", feed_response_data)
...
#     # def lambda_handler():
    ...
    try:
        ...
        if product_id_list is not None and len(product_id_list) != 0:
            ...
            query1 = "select product_id,amazon_portal_sku,amazon_account_id from amazon_amazonproducts where product_id in (" + product_id_str + ")"
            ...
            query2 = "select product_id,warehouse,stock from api_masterstock where product_id in (" + product_id_str + ")"
            sql1 = pd.read_sql(query2, engine2)
            # print("sql==",sql)
            sql1.to_csv('/tmp/a2.csv', index=False)
            print("*****successfull******")

            df115 = pd.read_csv("/tmp/a1.csv")
            df125 = pd.read_csv("/tmp/a2.csv")
            df225 = pd.merge(left=df115, right=df125, left_on='product_id', right_on='product_id')
            df225.to_csv('/tmp/a4.csv', index=False)
            data = pd.read_csv('/tmp/a4.csv')
            ###to take data belongs to warehouse 4 only
            data1 = data[data['warehouse'] == 4]
            engine1.dispose()
            engine2.dispose()
        ...
        ###to convert dataframe in to a tuple of a list
        records = data1.to_records(index=False)
        # print(result)
        amazon_response = list(records)
        print("amazon_response=====>", amazon_response)
        products_column_names = list(data.columns)
        print("products_column_names====>", products_column_names)
        ...
        if len(amazon_response) != 0:
            ...
            for single_record in amazon_response:
                ...
                if int(single_record[2]) not in amazon_results_data:
                    amazon_results_data[int(single_record[2])] = []

                # Create dic list based on account id
                amazon_results_data[int(single_record[2])].append(dict(zip(products_column_names, single_record)))
        ...
        if len(amazon_results_data) != 0:
            ...
            for account_id in amazon_results_data:
                ...
                account_id1=17
                account = access_token_list.get(account_id1, None)
                products_data = amazon_results_data.get(17)
        ...
        file_to = '/buymore2/stock_update/amazon/' + file_name
