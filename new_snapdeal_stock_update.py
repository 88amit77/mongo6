...
def get_paytm_product(product_id,cur_products):
    ...
    conn_products1 = psycopg2.connect(database=db_name_products, user=name, password=password,
                                     host=rds_host, port="5432")
    cur_products1 = conn_products1.cursor()
    cur_products1.execute(product_query)
    product = cur_products1.fetchone()
...
def main():
    ...
    paytm_fieldnames = ['flipkart_portal_sku', 'product_id', 'location', 'inventory', 'status', 'error_code',
                           'error_desc',
    ...
    products_query = "SELECT product_id, buymore_sku from master_masterproduct where product_id in ('257007','172519')"
    ...
    for item in binreco:
        ...
        print("product_id==>",product_id)
        paytm_product = get_paytm_product(product_id, cur_products)
